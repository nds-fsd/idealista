...
import User from "../../assets/iconsSVG/user-2-svgrepo-com.svg";
...
const Chat = () => {
    ...
    useEffect(() => {
        if (!toUser || !socketContext) navigate("/");
    ...
    return (
        <div className={styles.chatContainer}>
            <div className={styles.chatList}>
                <span style={{ fontSize: "29px", fontWeight: "600" }}> Mis Mensajes</span>
                <ul>
                    {contactList.map((e) => (
                        <button
                            onClick={() => {
                                navigate(`/chat?user=${e.id}`);
                            }}
                            key={e.id}
                        >
            ...
            <div className={styles.messagesContainer}>
                <div>
                    <ul style={{ flexDirection: "column", gap: "10px", marginBottom: "12px", flexWrap: "nowrap", height: "600px", overflow: "auto" }}>{messages.length > 0 && messages.filter((m) => m.userSender == toUser || m.userReceiver == toUser).map((message) => <Message user={message.userSender} text={message.content} loggedUser={user._id} key={message._id} />)}</ul>
                </div>
                <div className={styles.chatbox}>
                    ...
                    <div style={{ display: "flex" }}>
                        <button className={styles.contact_button} onClick={handleClick}>
