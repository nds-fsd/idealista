...
const validateRegister = async (req, res, next) => {
    ...
    if (!validName.test(name)) {
        return res.status(400).json({ error: {name: "El nombre debe tener al menos 3 caracteres" }})
    }
    ...
    if (!emailFormat.test(email)) {
        return res.status(400).json({ error: { email: "Este no es un email válido" }})
    }
    ...
    if (!validatePassword.test(password)) {
        return res.status(400).json({ error: { password: 'La contraseña debe tener al menos: 8 caracteres, una letra mayúscula, una letra minúscula y un número' }})
    }
